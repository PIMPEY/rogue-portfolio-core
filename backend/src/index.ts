...
app.get('/api/investments/:id', asyncHandler(async (req, res) => {
  ...
  const transformed = {
    investment: {
      ...
      calculatedRunwayMonths: investment.calculatedRunwayMonths,
      cashAtSnapshot: investment.cashAtSnapshot,
      snapshotDate: investment.snapshotDate?.toISOString() || null,
      liquidityExpectation: investment.liquidityExpectation,
      expectedLiquidityDate: investment.expectedLiquidityDate?.toISOString() || null,
