...
export default function SimpleMVP() {
  ...
  const createAndAnalyze = async () => {if (!investment.companyName || !investment.committedCapital || !investment.localEquivalent) {setError("Please fill in all required fields (Company Name, Committed Capital, and Local Equivalent)");return;}setUploading(true);setError("");setSuccess("");try {const BACKEND_URL = process.env.NEXT_PUBLIC_BACKEND_URL || "http://localhost:3001";const response = await fetch(\`\${BACKEND_URL}/api/investments/create\`, {method: "POST",headers: {"Content-Type": "application/json"},body: JSON.stringify({investment: {...investment,committedCapitalLcl: parseFloat(investment.committedCapital),currentFairValueEur: parseFloat(investment.localEquivalent),icApprovalDate: new Date().toISOString(),investmentExecutionDate: new Date().toISOString(),icReference: \`IC-\${Date.now()}\`},files: documents.map((doc) => ({fileName: doc.fileName,fileSize: doc.fileSize,filePath: \`local://\${doc.fileName}\`}))})});if (!response.ok) {const errorData = await response.json();const errorMessage = errorData.error || errorData.message || JSON.stringify(errorData);throw new Error(errorMessage);}const result = await response.json();if (documents.length > 0) {await analyzeWithChatGPT();}setShowReviewPopup(true);setSuccess("Investment created successfully!");} catch (err: any) {setError("Failed to create investment: " + err.message);} finally {setUploading(false);}};\n\n  const exportToJSON = () => {
    if (!analysis) return;
